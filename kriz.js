...
const { smsg, formatp, tanggal, formatDate, getTime, isUrl, sleep, clockString, runtime, fetchJson, parsedJid, getJson, getBuffer, jsonformat, format, parseMention, getRandom, getGroupAdmins } = require('./lib/myfunc')
...
let waveform = Array.from({length: 15}, () => Math.floor(Math.random() * 100)); // use this for fancy: [0,99,0,99,0,99]
                ...
                for (let i of anu) {
                   ...
                   if (/audio/.test(mime)) {
                    kriz.sendMessage(i, {audio: buffer, mimetype: 'audio/mpeg', ptt:true, waveform,
contextInfo:{
externalAdReply:{
 title: ' WʜᴀᴛꜱKʀɪᴢ AI',
		body: 'ᴛᴇᴀᴍ ᴛᴏxɪᴄ❗️',
